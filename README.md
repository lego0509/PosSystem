...
- 受付一時停止トグルと赤帯表示を実装。
- 注文確定後に 3 桁番号を大きく表示し案内。
- 売り切れ指定された商品は「SOLD OUT」表示になり、タップできない状態で残りのメニューと並びます。

### キッチンボード `/kds`
- 1 画面に全注文をまとめたシングルボード。状態順（待ち→調理中→完成）で自動整列。
- 表示件数は数値指定（例: 4 / 8 / 12 / 16 件）で調整でき、ページ送りで古い順に確認可能。
- 無料オプションを色付きバッジで表現。経過タイマーを mm:ss で自動更新。
- 検索（番号・名前）とカテゴリフィルタ、Undo ボタン、状態変更時の効果音を搭載。
- 「完成→受け渡し完了」の 2 ボタン運用。直近 10 件の受け渡し履歴を右側に表示し、誤操作時は戻すことができます。

### 呼び出し `/call`
- 完成状態の番号を固定サイズの 3×4 グリッド（最大 12 件、あふれた分は待機）で表示し、新規完成時に数秒ハイライト＆効果音。
- 番号のタップ／Enter／Space で再掲対象に選択。再掲ボタンで再度ハイライト。
- 操作は基本的に表示のみ。受け渡し完了はキッチンボードから行い、最近 10 件の履歴を下部に小さく表示します。
...
- 各商品を「売り切れ」に設定するチェックボックスを追加。設定すると POS 側で選択不可＆SOLD OUT 表示になります。
