...
`localStorage` に保存されたデータを初期化したい場合は、ブラウザの開発者ツールから `school-fes-pos-state` と `school-fes-pos-products-v1` を削除するか、`src/storage.js` の `clearAll()` をコンソールで呼び出し、商品は `/catalog` の「初期データに戻す」を使用してください。
